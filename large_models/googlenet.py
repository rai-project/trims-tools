...
for ii in range(1, 6):
    input_mult = ii
    input_dim = input_mult * 227
    sym = get_symbol()
    mod = mx.mod.Module(sym)
    mod.bind(data_shapes=[('data', (1,3,input_dim,input_dim))], label_shapes=[('softmax', (1,))])
    mod.init_params()

    os.mkdir('/models/large_googlenet_%dx%d' %(input_dim, input_dim))
    prefix = '/models/large_googlenet_%dx%d/googlenet.' %(input_dim, input_dim)
