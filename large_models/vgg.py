...
for ii in range(1, 6):
    input_mult = ii
    input_dim = input_mult * 224
    sym = get_symbol(1000, 16)
    mod = mx.mod.Module(sym)
    mod.bind(data_shapes=[('data', (1,3,input_dim,input_dim))], label_shapes=[('softmax_label', (1,))])
    mod.init_params()

    os.mkdir('/models/large_vgg16_%dx%d' %(input_dim, input_dim))
    prefix = '/models/large_vgg16_%dx%d/vgg.' %(input_dim, input_dim)
